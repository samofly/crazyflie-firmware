...
#define EXTRA_PRECISION                                                        \
  5 // trick to add more precision to the pressure and temp readings
...
bool ms5611Init(I2C_TypeDef *i2cPort) {
  ...
  vTaskDelay(M2T(5));
  if (ms5611ReadPROM() == FALSE) // reads the PROM into object variables for
                                 // later use
      {
    return FALSE;
  }
...
bool ms5611SelfTest(void) {
  ...
  if (ms5611EvaluateSelfTest(MS5611_ST_PRESS_MIN, MS5611_ST_PRESS_MAX, pressure,
                             "pressure") &&
...
bool ms5611ReadPROM() {
  ...
  for (i = 0; i < MS5611_PROM_REG_COUNT; i++) {
    // start read sequence
    status =
        i2cdevWriteByte(I2Cx, devAddr, I2CDEV_NO_MEM_ADDR,
