...
bool i2cdevRead(I2C_TypeDef *I2Cx, uint8_t devAddress, uint8_t memAddress,
  ...
  if (memAddress != I2CDEV_NO_MEM_ADDR) {
    status = I2C_Master_BufferWrite(I2Cx, &memAddress, 1, INTERRUPT,
                                    devAddress << 1, I2C_TIMEOUT);
  }
  if (status) {
    // TODO: Fix DMA transfer if more then 3 bytes
    status = I2C_Master_BufferRead(I2Cx, (uint8_t *)data, len, INTERRUPT,
                                   devAddress << 1, I2C_TIMEOUT);
  }
...
bool i2cdevWrite(I2C_TypeDef *I2Cx, uint8_t devAddress, uint8_t memAddress,
  ...
  if (memAddress != I2CDEV_NO_MEM_ADDR) {
    ...
    status = I2C_Master_BufferWrite(I2Cx, data, len, INTERRUPT, devAddress << 1,
                                    I2C_TIMEOUT);
  }
