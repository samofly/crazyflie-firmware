 ...
 * @param Mode: Polling or DMA or Interrupt having the highest priority in the
 * application.
...
ErrorStatus I2C_Master_BufferRead(I2C_TypeDef *I2Cx, uint8_t *pBuffer,
  ...
  /* Enable I2C errors interrupts (used in all modes: Polling, DMA and
   * Interrupts */
  I2Cx->CR2 |= I2C_IT_ERR;
  ...
  if (Mode == DMA) /* I2Cx Master Reception using DMA */
    ...
    /* Make sure that the STOP bit is cleared by Hardware before CR1 write
     * access */
    while ((I2Cx->CR1 & 0x200) == 0x200)
      ;
  }
      /* I2Cx Master Reception using Interrupts with highest priority in an
         application */
      else {
    /* Enable EVT IT*/
    I2Cx->CR2 |= I2C_IT_EVT;
    ...
    /* Wait until the START condition is generated on the bus: START bit is
     * cleared by hardware */
...
ErrorStatus I2C_Master_BufferWrite(I2C_TypeDef *I2Cx, uint8_t *pBuffer,
  ...
  if (Mode == DMA) /* I2Cx Master Transmission using DMA */
    ...
    while ((I2Cx->CR1 & 0x200) == 0x200)
      ...
      /* I2Cx Master Transmission using Interrupt with highest priority in the
         application */
      else {
    /* Enable EVT IT*/
    I2Cx->CR2 |= I2C_IT_EVT;
    ...
    I2Cx->CR1 |= CR1_START_Set;
    Timeout = timeoutMs * I2CDEV_LOOPS_PER_MS;
    /* Wait until the START condition is generated on the bus: the START bit is
     * cleared by hardware */
    while ((I2Cx->CR1 & 0x100) == 0x100 && Timeout) {
      Timeout--;
    }
...
void I2C_Slave_BufferReadWrite(I2C_TypeDef *I2Cx, I2C_ProgrammingModel Mode) {
  ...
  if (Mode == DMA) /* I2Cx Slave Transmission using DMA */
    ...
    I2Cx->CR2 |= CR2_DMAEN_Set;
  } else /* I2Cx Slave Transmission using Interrupt with highest priority in the
            application */
      {
...
void I2C_LowLevel_Init(I2C_TypeDef *I2Cx) {
  ...
  if (I2Cx == I2C1) { /* I2C1 TX DMA Channel configuration */
    DMA_DeInit(I2C1_DMA_CHANNEL_TX);
    I2CDMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)I2C1_DR_Address;
    I2CDMA_InitStructure.DMA_MemoryBaseAddr =
        (uint32_t)0; /* This parameter will be configured durig communication */
    I2CDMA_InitStructure.DMA_DIR =
        DMA_DIR_PeripheralDST; /* This parameter will be configured durig
                                  communication */
    I2CDMA_InitStructure.DMA_BufferSize =
        0xFFFF; /* This parameter will be configured durig communication */
...
void i2cInterruptHandlerI2c1(void) {
  ...
  if ((SR1Register & 0x0001) == 0x0001) {
    /* Send the slave address for transmssion or for reception (according to the
     configured value
  ...
  if ((SR2Register & 0x0001) == 0x0001) {
    ...
    if ((SR1Register & 0x0040) == 0x0040) {
      ...
      NumbOfBytes1--;
      /* If it remains only one byte to read, disable ACK and program the STOP
       * (EV7_1) */
      if (NumbOfBytes1 == 1) {
        /* Clear ACK */
        I2C1->CR1 &= CR1_ACK_Reset;
