...
prvInsertTimerInActiveList(xTIMER *pxTimer, portTickType xNextExpiryTime,
 ...
 */
static portTickType
prvSampleTimeNow(portBASE_TYPE *pxTimerListsWereSwitched) PRIVILEGED_FUNCTION;
 ...
 */
static portTickType
prvGetNextExpireTime(portBASE_TYPE *pxListWasEmpty) PRIVILEGED_FUNCTION;
 ...
 */
static void
prvProcessTimerOrBlockTask(portTickType xNextExpireTime,
...
portBASE_TYPE xTimerCreateTimerTask(void) {
  ...
  if (xTimerQueue != NULL) {
#if (INCLUDE_xTimerGetTimerDaemonTaskHandle == 1)
    {
      ...
      xReturn =
          xTaskCreate(prvTimerTask, (const signed char *)"Tmr Svc",
                      (unsigned short)configTIMER_TASK_STACK_DEPTH, NULL,
                      ((unsigned portBASE_TYPE)configTIMER_TASK_PRIORITY) |
                          portPRIVILEGE_BIT,
                      &xTimerTaskHandle);
    }
#else
...
portBASE_TYPE xTimerGenericCommand(
    xTimerHandle xTimer, portBASE_TYPE xCommandID, portTickType xOptionalValue,
...
static portTickType prvSampleTimeNow(portBASE_TYPE *pxTimerListsWereSwitched) {
  portTickType xTimeNow;
  PRIVILEGED_DATA static portTickType xLastTime =
      (portTickType)0U; /*lint !e956 Variable is only accessible to one task. */
...
static void prvProcessReceivedCommands(void) {
  ...
  while (
      ...
      pdFAIL) /*lint !e603 xMessage does not have to be initialised as it is
                 passed out, not in, and it is not used unless xQueueReceive()
                 returns pdTRUE. */
      {
    ...
    case tmrCOMMAND_START:
      ...
      if (prvInsertTimerInActiveList(
              ...
              xTimeNow, xMessage.xMessageValue) == pdTRUE) {
        /* The timer expired before it was added to the active timer
        list.  Process it now. */
        pxTimer->pxCallbackFunction((xTimerHandle)pxTimer);
        ...
        if (pxTimer->uxAutoReload == (unsigned portBASE_TYPE)pdTRUE) {
          xResult = xTimerGenericCommand(
              pxTimer, tmrCOMMAND_START,
              xMessage.xMessageValue + pxTimer->xTimerPeriodInTicks, NULL,
              tmrNO_DELAY);
          configASSERT(xResult);
          (void)xResult;
        }
...
static void prvCheckForValidListAndQueue(void) {
  ...
  taskENTER_CRITICAL();
  {
    if (xTimerQueue == NULL) {
      ...
      xTimerQueue =
          xQueueCreate((unsigned portBASE_TYPE)configTIMER_QUEUE_LENGTH,
