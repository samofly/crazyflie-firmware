...
typedef struct QueueDefinition {
  ...
  union /* Use of a union is an exception to the coding standard to ensure two
    ...
    uxRecursiveCallCount; /*< Maintains a count of the numebr of times a
  ...
  unsigned portBASE_TYPE uxLength; /*< The length of the queue defined as the
                                      number of items it will hold, not the
                                      number of bytes. */
  unsigned portBASE_TYPE uxItemSize; /*< The size of each items that the queue
                                        will hold. */

  volatile signed portBASE_TYPE
  xRxLock; /*< Stores the number of items received from the queue (removed from
              the queue) while the queue was locked.  Set to queueUNLOCKED when
              the queue is not locked. */
  volatile signed portBASE_TYPE xTxLock; /*< Stores the number of items
                                            transmitted to the queue (added to
                                            the queue) while the queue was
                                            locked.  Set to queueUNLOCKED when
                                            the queue is not locked. */
...
prvIsQueueEmpty(const xQUEUE *pxQueue) PRIVILEGED_FUNCTION;
 ...
 */
static signed portBASE_TYPE
prvIsQueueFull(const xQUEUE *pxQueue) PRIVILEGED_FUNCTION;
 ...
 */
static void
prvCopyDataFromQueue(xQUEUE *const pxQueue,
...
prvNotifyQueueSetContainer(const xQUEUE *const pxQueue,
...
portBASE_TYPE xQueueGenericReset(xQueueHandle xQueue, portBASE_TYPE xNewQueue) {
  ...
  taskENTER_CRITICAL();
  {
    ...
    pxQueue->u.pcReadFrom =
        pxQueue->pcHead + ((pxQueue->uxLength - (unsigned portBASE_TYPE)1U) *
                           pxQueue->uxItemSize);
  ...
  if (uxQueueLength > (unsigned portBASE_TYPE)0) {
    ...
    if (pxNewQueue != NULL) {
      ...
      xQueueSizeInBytes = (size_t)(uxQueueLength * uxItemSize) +
                          (size_t)1; /*lint !e961 MISRA exception as the casts
...
portBASE_TYPE xQueueGiveMutexRecursive(xQueueHandle xMutex) {
  ...
  if (pxMutex->pxMutexHolder == (void *)
      xTaskGetCurrentTaskHandle()) /*lint !e961 Not a redundant cast as
                                      xTaskHandle is a typedef. */
      {
...
portBASE_TYPE xQueueTakeMutexRecursive(xQueueHandle xMutex,
  ...
  if (pxMutex->pxMutexHolder == (void *)
      xTaskGetCurrentTaskHandle()) /*lint !e961 Cast is not redundant as
                                      xTaskHandle is a typedef. */
      {
    (pxMutex->u.uxRecursiveCallCount)++;
    xReturn = pdPASS;
...
xQueueGenericSend(xQueueHandle xQueue, const void *const pvItemToQueue,
...
signed portBASE_TYPE
xQueueAltGenericSend(xQueueHandle xQueue, const void *const pvItemToQueue,
                     portTickType xTicksToWait, portBASE_TYPE xCopyPosition) {
  signed portBASE_TYPE xEntryTimeSet = pdFALSE;
  xTimeOutType xTimeOut;
  xQUEUE *const pxQueue = (xQUEUE *)xQueue;
...
signed portBASE_TYPE
xQueueAltGenericReceive(xQueueHandle xQueue, void *const pvBuffer,
...
#if (configUSE_MUTEXES == 1)
          {
            if (pxQueue->uxQueueType == queueQUEUE_IS_MUTEX) {
              ...
              pxQueue->pxMutexHolder =
                  (signed char *)xTaskGetCurrentTaskHandle();
            }
          }
#endif
...
signed portBASE_TYPE
xQueueGenericSendFromISR(xQueueHandle xQueue, const void *const pvItemToQueue,
...
signed portBASE_TYPE
xQueueGenericReceive(xQueueHandle xQueue, const void *const pvBuffer,
                     portTickType xTicksToWait, portBASE_TYPE xJustPeeking) {
  signed portBASE_TYPE xEntryTimeSet = pdFALSE;
  xTimeOutType xTimeOut;
  signed char *pcOriginalReadPosition;
...
#if (configUSE_MUTEXES == 1)
          {
            if (pxQueue->uxQueueType == queueQUEUE_IS_MUTEX) {
              ...
              pxQueue->pxMutexHolder = (signed char *)
                  xTaskGetCurrentTaskHandle(); /*lint !e961 Cast is not
                                                  redundant as xTaskHandle is a
                                                  typedef. */
            }
          }
#endif
...
signed portBASE_TYPE
xQueueReceiveFromISR(xQueueHandle xQueue, const void *const pvBuffer,
                     signed portBASE_TYPE *pxHigherPriorityTaskWoken) {
  signed portBASE_TYPE xReturn;
  unsigned portBASE_TYPE uxSavedInterruptStatus;
  xQUEUE *const pxQueue = (xQUEUE *)xQueue;
...
static void prvCopyDataToQueue(xQUEUE *pxQueue, const void *pvItemToQueue,
  ...
  if (pxQueue->uxItemSize == (unsigned portBASE_TYPE)0) {
    ...
    (void)
        memcpy((void *)pxQueue->pcWriteTo,
               pvItemToQueue, (size_t)pxQueue
                                  ->uxItemSize); /*lint !e961 !e418 MISRA
                                                    ...
                                                    if the copy size is 0. */
    pxQueue->pcWriteTo += pxQueue->uxItemSize;
    if (pxQueue->pcWriteTo >= pxQueue->pcTail) /*lint !e946 MISRA exception
                                                  justified as comparison of
                                                  pointers is the cleanest
                                                  solution. */
        {
      pxQueue->pcWriteTo = pxQueue->pcHead;
    }
    ...
    (void)memcpy((void *)pxQueue->u.pcReadFrom, pvItemToQueue,
                 (size_t)pxQueue->uxItemSize); /*lint !e961 MISRA exception as
                                                  the casts are only redundant
                                                  for some ports. */
    pxQueue->u.pcReadFrom -= pxQueue->uxItemSize;
    if (pxQueue->u.pcReadFrom < pxQueue->pcHead) /*lint !e946 MISRA exception
                                                    justified as comparison of
                                                    pointers is the cleanest
                                                    solution. */
        {
...
static void prvCopyDataFromQueue(xQUEUE *const pxQueue,
  ...
  if (pxQueue->uxQueueType != queueQUEUE_IS_MUTEX) {
    ...
    (void)
        memcpy((void *)pvBuffer, (void *)pxQueue->u.pcReadFrom,
               (
                   size_t)pxQueue->uxItemSize); /*lint !e961 !e418 MISRA
                                                   exception as the casts are
                                                   only redundant for some
...
signed portBASE_TYPE
xQueueCRSendFromISR(xQueueHandle xQueue, const void *pvItemToQueue,
...
signed portBASE_TYPE
xQueueCRReceiveFromISR(xQueueHandle xQueue, void *pvBuffer,
...
portBASE_TYPE xQueueRemoveFromSet(xQueueSetMemberHandle xQueueOrSemaphore,
  ...
  if (pxQueueOrSemaphore->pxQueueSetContainer != xQueueSet) {
    ...
    xReturn = pdFAIL;
  } else if (pxQueueOrSemaphore->uxMessagesWaiting !=
             (unsigned portBASE_TYPE)0) {
...
xQueueSetMemberHandle xQueueSelectFromSet(xQueueSetHandle xQueueSet,
  ...
  (void)xQueueGenericReceive(
      (xQueueHandle)xQueueSet, &xReturn, xBlockTimeTicks,
      pdFALSE); /*lint !e961 Casting from one typedef to another is not
                   redundant. */
  return xReturn;
}
