...
xPendingReadyList; /*< Tasks that have been readied while the scheduler was
                      suspended.  They will be moved to the ready list when the
...
prvListTaskWithinSingleList(xTaskStatusType *pxTaskStatusArray, xList *pxList,
...
static unsigned short prvTaskCheckFreeStackSpace(
  ...
  taskENTER_CRITICAL();
  {
    ...
    if (uxCurrentBasePriority != uxNewPriority) {
      ...
      listSET_LIST_ITEM_VALUE(
          ...
          ((portTickType)configMAX_PRIORITIES -
           (portTickType)uxNewPriority)); /*lint !e961 MISRA exception as the
...
/* Add the idle task at the lowest priority. */
#if (INCLUDE_xTaskGetIdleTaskHandle == 1)
  {
    ...
    xReturn = xTaskCreate(prvIdleTask, (signed char *)"IDLE",
                          tskIDLE_STACK_SIZE, (void *)NULL,
...
#else
  {
    ...
    xReturn = xTaskCreate(prvIdleTask, (signed char *)"IDLE",
                          tskIDLE_STACK_SIZE, (void *)NULL,
                          (tskIDLE_PRIORITY | portPRIVILEGE_BIT),
                          NULL); /*lint !e961 MISRA exception, justified as it
  ...
  vTaskSuspendAll();
  {
    ...
    if (uxArraySize >= uxCurrentNumberOfTasks) {
      ...
      do {
        uxQueue--;
        uxTask +=
            prvListTaskWithinSingleList(&(pxTaskStatusArray[uxTask]),
                                        &(pxReadyTasksLists[uxQueue]), eReady);

      } while (
          uxQueue >
          (unsigned portBASE_TYPE)tskIDLE_PRIORITY); /*lint !e961 MISRA
      ...
      uxTask += prvListTaskWithinSingleList(
          &(pxTaskStatusArray[uxTask]), (xList *)pxDelayedTaskList, eBlocked);
    ...
    if (*pxTicksToWait == portMAX_DELAY) {
      xReturn = pdFALSE;
        ...
        if (
                ...
                (xConstTickCount >= pxTimeOut
                                        ->xTimeOnEntering)) /*lint !e525
                                                               Indentation
                                                               preferred as is
                                                               to make code
                                                               within
...
static void prvInitialiseTCBVariables(tskTCB *pxTCB,
  ...
  listSET_LIST_ITEM_VALUE(
      &(pxTCB->xEventListItem),
      (portTickType)configMAX_PRIORITIES -
          (portTickType)uxPriority); /*lint !e961 MISRA exception as the casts
...
static tskTCB *prvAllocateTCBAndStack(unsigned short usStackDepth,
  ...
  if (pxNewTCB != NULL) {
    ...
    pxNewTCB->pxStack = (portSTACK_TYPE *)pvPortMallocAligned(
        ...
        puxStackBuffer); /*lint !e961 MISRA exception as the casts are only
  ...
  if (listCURRENT_LIST_LENGTH(pxList) > (unsigned portBASE_TYPE)0) {
    ...
    do {
      ...
      pxTaskStatusArray[uxTask].pcTaskName =
          (const signed char *)&(pxNextTCB->pcTaskName[0]);
...
#if (configGENERATE_RUN_TIME_STATS == 1)
      {
        pxTaskStatusArray[uxTask].ulRunTimeCounter =
...
#else
      {
        pxTaskStatusArray[uxTask].usStackHighWaterMark =
            prvTaskCheckFreeStackSpace((unsigned char *)pxNextTCB->pxStack);
      }
#endif
    ...
    if (pxTCB->uxPriority < pxCurrentTCB->uxPriority) {
      ...
      listSET_LIST_ITEM_VALUE(
          ...
          (portTickType)configMAX_PRIORITIES - (portTickType)
              pxCurrentTCB->uxPriority); /*lint !e961 MISRA exception as the
                                            casts are only redundant for some
                                            ports. */
...
void vTaskList(signed char *pcWriteBuffer) {
  ...
  *pcWriteBuffer = 0x00;
  ...
  pxTaskStatusArray =
      pvPortMalloc(uxCurrentNumberOfTasks * sizeof(xTaskStatusType));
