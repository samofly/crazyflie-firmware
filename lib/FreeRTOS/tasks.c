...
typedef struct tskTaskControlBlock {
  ...
  portSTACK_TYPE *pxEndOfStack; /*< Points to the end of the stack on
                                   architectures where the stack grows up from
                                   low memory. */
#endif

#if (portCRITICAL_NESTING_IN_TCB == 1)
  unsigned portBASE_TYPE uxCriticalNesting; /*< Holds the critical section
                                               nesting depth for ports that do
                                               not maintain their own count in
                                               the port layer. */
#endif

#if (configUSE_TRACE_FACILITY == 1)
  unsigned portBASE_TYPE uxTCBNumber; /*< Stores a number that increments each
  ...
  unsigned portBASE_TYPE uxBasePriority; /*< The priority last assigned to the
                                            task - used by the priority
  ...
  unsigned long ulRunTimeCounter; /*< Stores the amount of time the task has
...
PRIVILEGED_DATA static xList *volatile pxDelayedTaskList; /*< Points to the
                                                             ...
                                                             currently being
                                                             used. */
PRIVILEGED_DATA static xList *volatile pxOverflowDelayedTaskList; /*< Points to
                                                                     the delayed
                                                                     task list
                                                                     currently
                                                                     being used
...
xTasksWaitingTermination; /*< Tasks that have been deleted - but the their
...
PRIVILEGED_DATA static xList xSuspendedTaskList; /*< Tasks that are currently
                                                    suspended. */
...
PRIVILEGED_DATA static unsigned long ulTaskSwitchedInTime =
    0UL; /*< Holds the value of a timer/counter the last time a task was
            switched in. */
PRIVILEGED_DATA static unsigned long
ulTotalRunTime; /*< Holds the total amount of execution time as defined by the
                   run time counter clock. */
static void prvGenerateRunTimeStatsForTasksInList(
    const signed char *pcWriteBuffer, xList *pxList,
    unsigned long ulTotalRunTimeDiv100) PRIVILEGED_FUNCTION;
 ...
 */
static void
prvAddCurrentTaskToDelayedList(portTickType xTimeToWake) PRIVILEGED_FUNCTION;
...
static unsigned short usTaskCheckFreeStackSpace(
    const unsigned char *pucStackByte) PRIVILEGED_FUNCTION;
...
xTaskGenericCreate(pdTASK_CODE pxTaskCode, const signed char *const pcName,
...
required by the port. */
#if (portSTACK_GROWTH < 0)
    {
      ...
      pxTopOfStack =
          (portSTACK_TYPE *)(((portPOINTER_SIZE_TYPE)pxTopOfStack) &
                             ((portPOINTER_SIZE_TYPE) ~portBYTE_ALIGNMENT_MASK));

      /* Check the alignment of the calculated top of stack is correct. */
      configASSERT((((unsigned long)pxTopOfStack &
...
the	top of stack variable is updated. */
    ...
    portALIGNMENT_ASSERT_pxCurrentTCB(
        (((unsigned long)pxNewTCB->pxTopOfStack &
          (unsigned long)portBYTE_ALIGNMENT_MASK) == 0UL));
...
eTaskState eTaskGetState(xTaskHandle xTask) {
  ...
  if (pxTCB == pxCurrentTCB) {
    ...
    {
      pxStateList =
          (xList *)listLIST_ITEM_CONTAINER(&(pxTCB->xGenericListItem));
    }
    taskEXIT_CRITICAL();
...
/* Add the idle task at the lowest priority. */
#if (INCLUDE_xTaskGetIdleTaskHandle == 1)
  {
    ...
    xReturn = xTaskCreate(
        prvIdleTask, (signed char *)"IDLE", tskIDLE_STACK_SIZE, (void *)NULL,
...
void vTaskList(signed char *pcWriteBuffer) {
  ...
  vTaskSuspendAll();
  {
    ...
    *pcWriteBuffer = (signed char)0x00;
    ...
    if (listLIST_IS_EMPTY(pxDelayedTaskList) == pdFALSE) {
      prvListTaskWithinSingleList(pcWriteBuffer, (xList *)pxDelayedTaskList,
                                  tskBLOCKED_CHAR);
    }

    if (listLIST_IS_EMPTY(pxOverflowDelayedTaskList) == pdFALSE) {
      prvListTaskWithinSingleList(
          pcWriteBuffer, (xList *)pxOverflowDelayedTaskList, tskBLOCKED_CHAR);
    }

#if (INCLUDE_vTaskDelete == 1)
    {
      if (listLIST_IS_EMPTY(&xTasksWaitingTermination) == pdFALSE) {
        prvListTaskWithinSingleList(pcWriteBuffer, &xTasksWaitingTermination,
                                    tskDELETED_CHAR);
      }
    }
#endif
...
#if (INCLUDE_vTaskSuspend == 1)
    {
      if (listLIST_IS_EMPTY(&xSuspendedTaskList) == pdFALSE) {
        prvListTaskWithinSingleList(pcWriteBuffer, &xSuspendedTaskList,
                                    tskSUSPENDED_CHAR);
      }
    }
#endif
  ...
  vTaskSuspendAll();
  {
    ...
    *pcWriteBuffer = (signed char)0x00;
    ...
    do {
      ...
      if (listLIST_IS_EMPTY(&(pxReadyTasksLists[uxQueue])) == pdFALSE) {
        prvGenerateRunTimeStatsForTasksInList(
            ...
            ulTotalRunTimeDiv100);
      }
    } while (uxQueue > (unsigned short)tskIDLE_PRIORITY);

    if (listLIST_IS_EMPTY(pxDelayedTaskList) == pdFALSE) {
      prvGenerateRunTimeStatsForTasksInList(
          pcWriteBuffer, (xList *)pxDelayedTaskList, ulTotalRunTimeDiv100);
    }
...
#if (INCLUDE_vTaskDelete == 1)
    {
      if (listLIST_IS_EMPTY(&xTasksWaitingTermination) == pdFALSE) {
        prvGenerateRunTimeStatsForTasksInList(
            pcWriteBuffer, &xTasksWaitingTermination, ulTotalRunTimeDiv100);
      }
    }
#endif
...
#if (INCLUDE_vTaskSuspend == 1)
    {
      if (listLIST_IS_EMPTY(&xSuspendedTaskList) == pdFALSE) {
        prvGenerateRunTimeStatsForTasksInList(
            pcWriteBuffer, &xSuspendedTaskList, ulTotalRunTimeDiv100);
      }
    }
#endif
  }
...
} /*lint !e715 pvParameters is not accessed but all task functions require the
     same prototype. */
/*-----------------------------------------------------------*/
...
/* Store the function name in the TCB. */
  ...
  pxTCB->pcTaskName
      [(unsigned short)configMAX_TASK_NAME_LEN - (unsigned short)1] =
          (signed char)'\0';
...
static void prvListTaskWithinSingleList(const signed char *pcWriteBuffer,
  ...
  do {
    ...
    {
      usStackRemaining =
          usTaskCheckFreeStackSpace((unsigned char *)pxNextTCB->pxEndOfStack);
    }
#else
    {
      usStackRemaining =
  ...
  do {
    ...
    if (ulTotalRunTimeDiv100 > 0UL) {
      ...
      if (pxNextTCB->ulRunTimeCounter == 0UL) {
        ...
        ulStatsAsPercentage =
            pxNextTCB->ulRunTimeCounter / ulTotalRunTimeDiv100;
