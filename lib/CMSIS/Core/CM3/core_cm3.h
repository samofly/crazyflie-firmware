...
static __INLINE void NVIC_SetPriorityGrouping(uint32_t PriorityGroup) {
  ...
  reg_value &=
      ~((0xFFFFU << 16) | (0x0F << 8)); /* clear bits to change               */
  reg_value =
      ((reg_value | NVIC_AIRCR_VECTKEY |
 ...
 * @param  uint32_t PreemptPriority is the preemptive priority value (starting
 *from 0)
 ...
 * @param  uint32_t* pPreemptPrio is the preemptive priority value (starting
 *from 0)
...
static __INLINE uint32_t SysTick_Config(uint32_t ticks) {
  ...
  NVIC_SetPriority(SysTick_IRQn,
                   (1 << __NVIC_PRIO_BITS) - 1); /* set Priority for Cortex-M0
                                                    System Interrupts */
  SysTick->VAL = (0x00); /* Load the SysTick Counter Value */
  SysTick->CTRL =
      (1 << SYSTICK_CLKSOURCE) | (1 << SYSTICK_ENABLE) |
