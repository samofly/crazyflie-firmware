...
static const uint8_t typeLength[] = {[LOG_UINT8] = 1, [LOG_UINT16] = 2,
                                     [LOG_UINT32] = 4, [LOG_INT8] = 1,
                                     [LOG_INT16] = 2, [LOG_INT32] = 4,
                                     [LOG_FLOAT] = 4, [LOG_FP16] = 2, };
...
static int logAppendBlock(int id, struct ops_setting *settings, int len) {
  ...
  for (i = 0; i < len; i++) {
    ...
    if ((currentLength + typeLength[settings[i].logType & 0x0F]) >
        LOG_MAX_LEN) {
    ...
    if (settings[i].id ^ 255) { // TOC variable
      ...
      if (varId < 0) {
        ERROR("Trying to add variable Id %d that does not exists.",
              settings[i].id);
        return ENOENT;
      }
