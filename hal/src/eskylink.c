...
static struct crtpLinkOperations eskyOp = { .setEnable = setEnable,
...
static void eskylinkInitPairing(void) {
  ...
  // Power the radio, Enable the DR interruption, set the radio in PRX mode with
  // 2bytes CRC
...
static void eskylinkInitPaired(int channel) {
  ...
  nrfWriteReg(REG_RX_ADDR_P0, address, 4);     // Pipe address == from pairing
                                               // packet
  nrfWrite1Reg(REG_EN_RXADDR, 0x01);
  nrfWrite1Reg(REG_FEATURE, 0x00); // No dynamic size payload
  nrfWrite1Reg(REG_DYNPD, 0x00);
...
struct crtpLinkOperations *eskylinkGetLink() { return &eskyOp; }
